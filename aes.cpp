...
unsigned char fixedMatrix[4][4] = {{0x02, 0x03, 0x01, 0x01},
                                   {0x01, 0x02, 0x03, 0x01},
                                   {0x01, 0x01, 0x02, 0x03},
                                   {0x03, 0x01, 0x01, 0x02}};
...
void shiftRows() {
  ...
  // forth row, left circular shift by 3
  row++;
  stateTuple.clear();
  stateTuple.push_back(state[row][3]);
  stateTuple.push_back(state[row][0]);
  ...
  for (int i = 0; i < 4; i++) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      for (int k = 0; k < 4; k++) {
        ...
        // cout << hex << (int)copyState[y][i] << " * " <<
        // (int)fixedMatrix[j][y] << endl;

        midResult = l1 + l2;

        if (l1 + l2 > 255) {
          // cout <<dec << int(l1) <<" + " <<(int)l2 << " = " << l1 + l2 <<
          // endl;
...
// Found solution to choose input type as hex or string through the below hex_to_string function
// non-consequential to AES Encryption, just gives option to not have to convert from hex to string before starting
...
int main() {
  ...
  while(stringOptions != "1" && stringOptions != "2")
  {
      cout << "\033[1;31mNot a valid key. Press 1 for string input, 2 for hex\033[0m\n";
      getline(cin, stringOptions);
  }
